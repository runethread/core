...
Runethread uses a **mixed licensing boundary**. Core implementation material defaults to the [PolyForm Perimeter License 1.0.1](LICENSE) and is source-available. The only prospective permissive grant is the **exact exception in [`LICENSING_BOUNDARY.json`](LICENSING_BOUNDARY.json)**, using the [MIT License](LICENSE-MIT) for the exact contract/bootstrap/generated byte identities recorded there. **Perimeter is the default everywhere else.** `AI_SETUP.md` future changes are outside that prospective exception, and the public memory template is not blanket-licensed by repository membership. Historical pre-transition material retains the MIT rights already granted. User-authored memory/project data is not licensed to Runethread by this policy. See [`LICENSING.md`](LICENSING.md) and [ADR-026](docs/adr/ADR-026-runethread-licensing-and-commercial-model.md) for the authoritative scope and transition rules.
