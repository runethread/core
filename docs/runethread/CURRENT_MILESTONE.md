...
- **Portable interoperability layer:** Core's MIT exception is an independently guarded exact allowlist, not a category that `ContractPaths()` may expand automatically. It consists of the exact 19 current contract files plus `AI_SETUP.md` and `runethread-bootstrap.json`; the exact Runethread-authored support bytes emitted into memory repositories at `.gitattributes`, `README.md`, `.github/workflows/validate.yml`, `.runethread/config.json`, and `.runethread/lock.json` have a separate narrow MIT output exception. Generator/runtime source remains Perimeter-covered.
